  ...
  render(){
    ...
    if(this.state.item.task && this.state.item.task.length>0){
      ...
      if(this.state.openAddChecklist){
        addAnItemChecklist = <div>
          <form onSubmit={this.handleAddNewItem}>
            <TextField onChange={this.onChangeNewItemChecklist} hintText="Add an item..." value={this.state.newItemChecklist}  />
            <FlatButton onTouchTap={()=>{this.setState({openAddChecklist:!this.state.openAddChecklist})} }  label={<ContentClear />}  />
          </form>
          </div>
      }else{
        addAnItemChecklist = <div onTouchTap={()=>{this.setState({openAddChecklist:true})} }><small style={{color:grey400}}>Add an item...</small></div>
      }
      ...
      this.state.item.need_checklist.forEach((item,i)=>{
          ...
          checkListItem.push(
            <div style={{display:'flex'}} key={i}>
              ...
              />
              <div style={{color:lightBlack}}><small>{item.name}</small></div>
            </div>
          )
      });
      var checkList =
